...
def autoversionlocal():
	...
	if nCompleto!="":
		if (rutajuntaAMEO in carpetasPermitidas) and nCompleto!="" and creaCarpeta:
			...
			if bienNombrado:
				if nCompleto!="" and int ( (os.path.split(nCompletoLocal)[1])[-6:] ) -  int ( (os.path.split(nCompleto)[1])[-6:] )>=1 and vA >= archsO[-1]:
				...
				elif nCompleto!="" and int ( (os.path.split(nCompletoLocal)[1])[-6:-3] ) -  int ( (os.path.split(nCompleto)[1])[-6:-3] )>=1 and vA <= archsO[-1]:
					if nCompleto[0]== "D" and nCompleto[1:]!=nCompletoLocal[1:]:
