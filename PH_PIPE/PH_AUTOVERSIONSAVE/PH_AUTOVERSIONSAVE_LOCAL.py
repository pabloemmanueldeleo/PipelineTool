...
def autoversionlocal():
	...
	carpetasPermitidas = ["M:/MAYA","Q:/MAYA","D:/MAYA","M:/PH_SCRIPTS","D:/PH_SCRIPTS"]
	...
	if nCompleto!="":
		if (rutajuntaAMEO in carpetasPermitidas) and nCompleto!="" and creaCarpeta:
			...
			if nCompletoLocal == '' and nCompleto!="" and not (bienNombrado):
			    mc.file(rename='D'+nCompleto[1:-3] +'_V001')
			    mc.file(f=True, save=True, defaultExtensions=True)
			...
			if bienNombrado:
				if nCompleto!="" and int ( (os.path.split(nCompletoLocal)[1])[-3:] ) -  int ( (os.path.split(nCompleto)[1])[-6:-3] )>=1 and vA >= archsO[-1]:
					...
					#incremental encuentra conflicto
				elif nCompleto!="" and int ( (os.path.split(nCompletoLocal)[1])[-3:] ) -  int ( (os.path.split(nCompleto)[1])[-6:-3] )>=1 and vA <= archsO[-1]:
					if nCompleto[0]== "D" and nCompleto[1:]!=nCompletoLocal[1:]:
						print "INCREMENTAL CONFLICTO"
						mensajeIncEncontrada='SE HA ENCONTRADO ARCHIVO/S EN D:'
						mc.warning (mensajeIncEncontrada)
						ventanaSaveUI()
		else:
