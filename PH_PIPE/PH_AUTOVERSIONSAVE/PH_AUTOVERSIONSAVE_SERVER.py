...
def abrirCarpetaUI():
    ...
    mc.window(winCrear3, title='PH_AUTOVERSIONSAVE SERVER 1.0', backgroundColor=[1,0.4,0.06],wh=[644, 150], titleBarMenu=1,toolbox=1,resizeToFitChildren=1,s=1 )
    ...
    #query de anchos de columnas
    anchoTotalColumnas = mc.columnLayout( layoutC_1 , q = 1 , width = 1 )  +  mc.columnLayout( layoutC_2 , q = 1 , width = 1 )  +  mc.columnLayout( layoutC_3 , q = 1 , width = 1 )
    altoVentana = mc.rowColumnLayout ( layoutRC_1 , q = 1 , height = 1)
    mc.window(winCrear3,e = 1 , width = anchoTotalColumnas )
    mc.window(winCrear3,e = 1 , height = altoVentana )
...
def copiarM():
    ...
    mc.warning ("COPIA COMO ULTIMA VERSION")
    mc.deleteUI(winCrear3)
...
def autoversionserver():
    ...
    if (nCompleto[:7] in carpetasPermitidas[0] or nCompleto[:7] in carpetasPermitidas[1] or nCompleto[:7] in carpetasPermitidas[2] or nCompleto[:7] in carpetasPermitidas[3] or carpetasPermitidas[4] in nCompleto)and existeCarpeta and nCompleto[:7] != "M:/MAYA":
        ...
        if nCompletoM == '' and nCompleto!="":
            ...
            mc.file(rename="D"+rutaArchivoM[1:-3]+'_V001.ma')
            mc.file(f=True, save=True, defaultExtensions=True)


            mc.sysFile (nCompleto,copy=rutaArchivoM[:-3] +'_V001.ma')
