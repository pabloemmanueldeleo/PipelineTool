...
def makeWindow():
    ...
    cl1=cmds.columnLayout(parent=Form,adjustableColumn = True,columnOffset=['both',5])
    tx1=cmds.text(parent=cl1,label='\nGRUPOS')
    ...
    tx3=cmds.text(parent=cl1,label='\nCAMARAS')
    tslBOX=cmds.textScrollList(camListBox, parent=cl1, allowMultiSelection=False, selectCommand="selectChanged()",h=150)
    cl2=cmds.columnLayout(parent=cl1,adjustableColumn = True,columnOffset=['both',15])
    rl3=cmds.rowLayout( parent=cl2,numberOfColumns=3,columnAlign=(3, 'center'))
    b5=cmds.button(parent=rl3, label='ACTUALIZAR',command="refreshGui()", bgc=(0.5,0.2,0.0) ,annotation='Si hay algun cambio de camaras, es neceasrio actualizar.')
    b1=cmds.button(parent=rl3, label='VER CAMARA', command="camLook()", bgc=(0.5,0.1,0.5),annotation='-Te lleva a la camara seleccionada y te pone los key en el timeline.\n-Si tiene animacion la seleccion hace un fill del timeline.')
    b0=cmds.button(parent=rl3, label='UNLOCK/LOCK', command="lockcams()", bgc=(0.2,0.8,0.0),annotation='Desbloquea y bloquea atributos de las camaras por seguridad.')
    cl3=cmds.columnLayout(parent=cl2,adjustableColumn = True,columnOffset=['both',5])
    tx5=cmds.text(parent=cl3,label='\nEXTRA TOOLS\n')
    rl2=cmds.rowLayout( parent=cl3,numberOfColumns=5)
    b6=cmds.button(parent=rl2, label='IMPORTCAM', command="importrig()", bgc=(0.9,0.5,0.0) ,annotation='Seleciona la camara la cual remplazar para importar el rig nuevo.\n*La Camara de pampa tiene que tener este nombre para importar UD??_E???_P??_CAM')
    ...
    b4=cmds.button(parent=rl2, label='PLAYBLAST', command="playBlastShot()", bgc=(0.1,0.5,0.0) ,annotation='Crea en tu directorio de escena un preview con los datos necesarios, con los frames que le das.')
    tx4=cmds.text(parent=cl3,label='\nHELP: Mantene puntero del raton arriba de cada\n boton para ver mas info.')
    allowedAreas = ['right', 'left']
    sc1=cmds.dockControl( label='PH_MANAGERCAM'+version,area='left', content=winName, allowedArea=allowedAreas)
