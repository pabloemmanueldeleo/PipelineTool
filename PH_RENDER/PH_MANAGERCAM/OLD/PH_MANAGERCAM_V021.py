...
def makeWindow():
    ...
    #box
    tslBOX=mc.textScrollList(camListBox, parent=cl1, allowMultiSelection=False, selectCommand="PH_MANAGERCAM.selectChanged()",h=150)
    cl2=mc.columnLayout(parent=cl1,adjustableColumn = True,columnOffset=['both',15])
    txx=mc.text(parent=cl2,label='\n')
    '''rl4=mc.rowLayout( parent=cl2,numberOfColumns=4,columnWidth4=(80, 80, 80,80),columnAlign=(1, 'right'))
    txa=mc.text(parent=rl4,label='\n1\n')
    txb=mc.text(parent=rl4,label='\n2\n')
    txd=mc.text(parent=rl4,label='\n3\n')
    txe=mc.text(parent=rl4,label='\n4\n')'''
    rl3=mc.rowLayout( parent=cl2,numberOfColumns=4,columnAlign=(4, 'center'))
    ...
    b0=mc.button(parent=rl3, label='UNLOCK/LOCK', command="PH_MANAGERCAM.lockcams()", bgc=(0.2,0.8,0.0),annotation='Desbloquea y bloquea atributos de las camaras por seguridad.')
    cl3=mc.columnLayout(parent=cl2,adjustableColumn = True,columnOffset=['both',5])
    tx5=mc.text(parent=cl3,label='\nEXTRA TOOLS\n')
    rl2=mc.rowLayout( parent=cl3,numberOfColumns=5)
    b6=mc.button(parent=rl2, label='IMPORTCAM', command="PH_MANAGERCAM.importrig()", bgc=(0.9,0.5,0.0) ,annotation='Seleciona la camara la cual remplazar para importar el rig nuevo.\n*La Camara de pampa tiene que tener este nombre para importar UD??_E???_P??_CAM')
    b3=mc.button(parent=rl2, label='CREAR SEQ', command="PH_MANAGERCAM.cameraSequenceShow()",bgc=(0.3,0.7,1),annotation='Crea Secuencia con las camaras de la escena.\nTiene que tener key los controles si o si.' )
    b3a=mc.button(parent=rl2, label='SHOW', command='PH_MANAGERCAM.cameraShow()',bgc=(0.3,0.7,1),annotation='Muestra la ventana de Camera Sequencer.' )
    b4=mc.button(parent=rl2, label='PLAYBLAST', command="PH_MANAGERCAM.playBlastShot()", bgc=(0.1,0.5,0.0) ,annotation='Crea en tu directorio de escena un preview con los datos necesarios, con los frames que le das.')
    tx4=mc.text(parent=cl3,label='\nHELP: Sobre cada boton tenes mas informacion.')
    allowedAreas = ['right', 'left']
    sc1=mc.dockControl( label='PH_MANAGERCAM'+version,area='right', content=winName, allowedArea=allowedAreas)
