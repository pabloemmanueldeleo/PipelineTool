...
def makeWindow():
    ...
    rl1=cmds.rowLayout( numberOfColumns=3)
    cl1=cmds.columnLayout(parent=rl1,adjustableColumn = True)
    tx1=cmds.text(parent=cl1,label='GRUPOS\n')
    tslTRF=cmds.textScrollList(camListTrf,parent=cl1, deleteKeyCommand="delSelected()", allowMultiSelection=False, selectCommand="selectTrf()",h=400,w=200)
    ...
    tslBOX=cmds.textScrollList(camListBox,parent=cl2, allowMultiSelection=False, selectCommand="selectChanged()",h=300,w=200)
    ...
    b3=cmds.button(parent=cl2, label='CREAR SEQ', command="sequence()",bgc=(0.3,0.7,1) )
    b4=cmds.button(parent=cl2, label='REMPLACE CAMS', command="importrig()", bgc=(0.9,0.5,0.0) )
