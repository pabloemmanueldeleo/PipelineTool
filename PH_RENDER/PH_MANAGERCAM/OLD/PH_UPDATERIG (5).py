...
# No conecta, bakea. Pura fuerza bruta.
...
objetosSeleccionados = cmds.ls('SCAM_*_Control','SCAM_*_control','SCAM_*_CONTROL')
indexSel = 0 ;
hayNombreMalo = False
noTieneKeys = False
objsConProb=[]
...
for cam in objetosSeleccionados:
    if (matchStr ('SCAM_E???_P??_Control',cam)==True and matchStr ('SCAM_E???_P??__Control',cam)==True and matchStr ('SCAM_E???_P??_control',cam)==True and matchStr ('SCAM_E???_P??__control',cam)==True):
        hayNombreMalo=True
