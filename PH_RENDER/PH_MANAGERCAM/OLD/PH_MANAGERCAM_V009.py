...
def selectTrf():
    ...
    excludeListA=['*_CAM','GRP','TRFShape','HCNS','TRFSH','_SCAMF']
    if cameras!=[]:
        #filtro trf de camaras
        for x in excludeListA:
            for item in cameras:
                if item.find(x) != -1:
                    cameras.remove(item)
    cameras.sort()
...
def initGui():
    ...
    trfCams = cmds.ls('SCAM*_E*_P*','C_E*_P*','C_E*_P*__GRP','C_E*_P*__*T','C_E*_P*CAM', type='transform')
    excludeListB=['_Control','control','Control','CAM','__CNT','TRF','HCNS','CAMSH','CAMShape','SCAM']
    if trfCams!=[]:
        #filtro trf de camaras
...
def makeWindow():
    ...
    cmds.window( winName,title=winName+version ,bgc=(0.56,0.56,0.56))
    rl1=cmds.rowLayout( numberOfColumns=3,columnWidth3=(150, 50, 150))
    cl1=cmds.columnLayout(parent=rl1,adjustableColumn = True,columnOffset=['both',12])
    tx1=cmds.text(parent=cl1,label='GRUPOS\n')
    tslTRF=cmds.textScrollList(camListTrf,parent=cl1, deleteKeyCommand="delSelected()", allowMultiSelection=False, selectCommand="selectTrf()",h=350,bgc=(0.56,0.56,0.56))
    tx4=cmds.text(parent=rl1,label='>\n\n>\n\n>\n')
    cl2=cmds.columnLayout(parent=rl1,adjustableColumn = True, columnOffset=['both',12],rowSpacing=5)
    tx3=cmds.text(parent=cl2,label='CAMARAS\n')
    tslBOX=cmds.textScrollList(camListBox, parent=cl2, allowMultiSelection=False, selectCommand="selectChanged()",h=150,w=100)
    b0=cmds.button(parent=cl2, label='LOCK/UNLOCK', command="lockcams()", bgc=(0.2,0.8,0.0))
    b1=cmds.button(parent=cl2, label='VER', command="camLook()", bgc=(0.5,0.1,0.5))
    ...
    b3=cmds.button(parent=rl2, label='CREAR SEQ', command="sequence()",bgc=(0.3,0.7,1) )
    b4=cmds.button(parent=rl2, label='REMPLACE CAMS', command="importrig()", bgc=(0.9,0.5,0.0) )
    b5=cmds.button(parent=rl2, label='ACTUALIZAR', command="refreshGui()", bgc=(0.5,0.2,0.0) )
    cmds.showWindow()
