    ...
    if cameras!=[]:
        ...
        for x in excludeListA+excludeListC+trfCams:
            for item in cameras:
                if item.find(x) != -1:
                    cameras.remove(item)
        #Ejequea si tiene animacion y prende el boton de fill
        for cam in cameras:
            if 'CNT' in cam:
                if haveAnim(cam)==1:
                    buttonAnimOn()
...
def makeWindow():
    ...
    b6=mc.button(parent=cl4, label='CAM->STEREO', command="PH_MANAGERCAM.importrig()", bgc=(0.9,0.5,0.0) ,annotation='-Seleciona las camaras comunes y remplaza por RIG STEREO\n*La Camara de pampa tiene que tener este nombre para importar UD??_E???_P??_CAM')
    b8=mc.button(parent=cl4, label='UPDATE-RIG', command="PH_MANAGERCAM.upRig()", bgc=(0.7,0.6,0.1) ,annotation='-Remplaza rigCamaras viejos por nuevos.\nEj: SCAM_ y _control')
