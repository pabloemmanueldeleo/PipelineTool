...
#Alfa de imagePlanes.
def imagePlaneAlphaGain():
	ipValor = mc.floatFieldGrp ('valorAlfa', q=1 , value1=1)
	iPlanes = mc.ls('imagePlane*')
	for ip in iPlanes:
		mc.setAttr ( (ip.split("->")[1]+".alphaGain") , ipValor)
...
def bakeCamGrp():
	objS = ""
	if len(mc.ls (type="shot"))>0:
		gruposCams = selectTrf()
	canalesParaBake = ['tx','ty','tz','rx','ry','rz','sx','sy','sz']
	if len(gruposCams)!=0:
		if len(mc.ls (type="shot"))>0:
			mel.eval("paneLayout -e -manage false $gMainPane")
			for o in gruposCams:
				shot_L= "L_" + (o.split("C_")[1])[:-3] + "SHO"
				shot_R= "R_" + (o.split("C_")[1])[:-3] + "SHO"
				#preparo el array con los canales para usar de argumento
				camaraAsignadaAlShotTRF_L = (mc.shot (shot_L, q=1, cc=1)).split("SH")[0]
				camaraAsignadaAlShotTRF_R = (mc.shot (shot_R, q=1, cc=1)).split("SH")[0]
				atts = mc.listAttr (camaraAsignadaAlShotTRF_L,k=1)
				for att in atts:
					mc.setAttr (camaraAsignadaAlShotTRF_L+"."+att, l=0)
					mc.setAttr (camaraAsignadaAlShotTRF_R+"."+att, l=0)

				inF = mc.shot (shot_L, q=1 , startTime =1)
				outF = mc.shot (shot_L, q=1 , endTime =1)
				print "\n\n O:\n", o
				for canalSolicitado in canalesParaBake:
					print "CANAL SOLICITADO:\n\n", canalSolicitado
					mel.eval('bakeResults -simulation true -t "'+str(inF)+':'+str(outF)+'" -sampleBy 1 -disableImplicitControl true -preserveOutsideKeys true -sparseAnimCurveBake false -removeBakedAttributeFromLayer false -bakeOnOverrideLayer false -minimizeRotation true -at "'+canalSolicitado+'" '+camaraAsignadaAlShotTRF_L+';')
					mel.eval('bakeResults -simulation true -t "'+str(inF)+':'+str(outF)+'" -sampleBy 1 -disableImplicitControl true -preserveOutsideKeys true -sparseAnimCurveBake false -removeBakedAttributeFromLayer false -bakeOnOverrideLayer false -minimizeRotation true -at "'+canalSolicitado+'" '+camaraAsignadaAlShotTRF_R+';')
			#EXPORTO
			for o in gruposCams:
				shot_L= "L_" + (o.split("C_")[1])[:-3] + "SHO"
				shot_R= "R_" + (o.split("C_")[1])[:-3] + "SHO"
				...
				camaraAsignadaAlShotTRF_R = (mc.shot (shot_R, q=1, cc=1)).split("SH")[0]
				inF = mc.shot (shot_L, q=1 , startTime =1)
				outF = mc.shot (shot_R, q=1 , endTime =1)
				mc.select (camaraAsignadaAlShotTRF_L,camaraAsignadaAlShotTRF_R)
				exportFbxManager( int(inF) , int(outF),camaraAsignadaAlShotTRF_L[2:])
				atts = mc.listAttr (camaraAsignadaAlShotTRF_L,k=1)
				for att in atts:
					mc.setAttr (camaraAsignadaAlShotTRF_L+"."+att, l=1)
					mc.setAttr (camaraAsignadaAlShotTRF_R+"."+att, l=1)
			mel.eval("paneLayout -e -manage true $gMainPane")
		else:
			mc.warning ("NO HAY SHOTS.")
	else:
	    mc.warning ("NO HAY GRUPOS SELECCIONADOS.")

def exportFbxManager( inFrame=1 , outFrame=2, nombreFbx="export___"):
	#TIENE QUE GUARDAR EN LA CARPETA DEL PROYECTO
	...
	ruta = mc.file(query=True,sceneName=True)
	ruta = os.path.split(ruta)[0]
	ruta = ruta+"/EXPORT_CAMS/"
	mc.sysFile (ruta, makeDir=True )
	mel.eval("FBXExport -f \""+ruta+"/"+nombreFbx+".fbx\" -s;")
...
def makeWindow():
    ...
    rl3=mc.rowLayout( parent=cl2,numberOfColumns=4,adjustableColumn4=3)
    ...
    global b4_2
    b4_2 = mc.floatFieldGrp('valorAlfa',parent=rl3,numberOfFields=1,annotation='-Ajusta el gain del alfa de los imagePlanes de la escena.', columnAlign2=['left','right'],columnWidth2=[30,30], label=' ALFA:', value1=1 , changeCommand="PH_MANAGERCAM.imagePlaneAlphaGain()" )

    #Extra
    tx5=mc.text(parent=cl1,label='\n-MORE-TOOLS-\n',font='boldLabelFont')
    cl3=mc.columnLayout(parent=cl1,adjustableColumn=True,columnAttach=('left',2))
    rl2=mc.rowLayout( parent=cl3,numberOfColumns=4,columnAttach3=("both","both",'both'))
    #Columna Extra 1
    ...
    #Columna Extra 2
    global esc
    cl5=mc.columnLayout(parent=rl2,rowSpacing=10)
    b0=mc.button(parent=cl5, label='LOCK ANM', w=60 , command="PH_MANAGERCAM.lockcams()", bgc=(0.2,0.8,0.0),annotation='-Desbloquea y bloquea atributos de las camaras por seguridad.')
    bBake = mc.button(parent=cl5, label='BAKE', w=60 , command="PH_MANAGERCAM.bakeCamGrp()", bgc=(1,0,0),annotation='-Bakea y exporta R y L de grupo\s seleccionados. Usa el cameraSequencer para obtener el intervalo a bakear. Exporta usando la ruta del .ma ')
    esc=mc.intFieldGrp ( parent=cl5,numberOfFields=1, columnAlign2=['left','right'],columnWidth2=[20,30], label='ESC', value1=000  )

    cl5b=mc.columnLayout(parent=rl2,rowSpacing=10,adjustableColumn=True)
    b0a=mc.button(parent=cl5b, label='LOCK PRO',  w=60 , command="PH_MANAGERCAM.lockcamsprop()", bgc=(0.7,0.2,0.1),annotation='-Desbloquea y bloquea atributos stereo de camaras y overscan (DisplayOptions).\nIMPORTANTE: Primero hablar con Diego.')
    mc.separator(height=24,visible=1,style="none")
    global pla
    pla=mc.textFieldGrp(parent=cl5b, columnAlign2=['left','right'],columnWidth2=[35,30], label='PLANO' , text = '00a')
    #Columna Extra 3
