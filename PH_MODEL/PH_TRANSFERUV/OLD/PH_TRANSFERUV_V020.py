...
def uvTranfer(mesh_source,mesh_target):
    ...
    cmds.setAttr(mesh_orig+'.'+'visibility', 1)
    ...
    cmds.setAttr(mesh_orig+'.'+'visibility', 0)
...
def uvTranferSelect():
    ...
    uvTranfer(mesh_target,mesh_source)

def UIuvTranfer():
    w=210
    h=50
    winName='PH_TRANSFERUV'
    version=' v1.0'
    nameWindow = 'uvTranfer'
    if cmds.window(nameWindow ,ex=True):
        cmds.deleteUI(nameWindow)
    cmds.window(nameWindow, title=winName+version, sizeable=False, resizeToFitChildren=True, wh=(w,h))
    ...
    b3=cmds.button(nameWindow + 'btn3', label=" SHOW ", command='PH_TRANSFERUV.uvWindows()',bgc=[1,.6,.3])
