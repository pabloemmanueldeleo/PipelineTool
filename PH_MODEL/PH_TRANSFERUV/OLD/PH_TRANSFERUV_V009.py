    ...
    if len(newListSrc) == len(newListTgt):
        if ':' in geoSrc or geoTgt:
            ...
            for i in range(len(newListSrc)):
                    ...
                    cmds.displaySmoothness(newListTgt[i], divisionsU= 0,divisionsV=0,pointsWire=4,pointsShaded=1,polygonObject=1)
                    ...
                    tfrAtt=cmds.transferAttributes(str(newListSrc[i]),newListTgt[i],
                                    ...
                                    flipUVs=False,
                                    colorBorders=True)
                    #cmds.setAttr(str(newListTft)+'.'+'intermediateObject', 1)
