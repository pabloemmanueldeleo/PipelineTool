...
def uvTranfer(mesh_source,mesh_target, history):
...
def checkBoxDeform():
    checkBoxDeform=cmds.checkBox(nameWindow + 'btn4',q = True, v = True)
...
def UIuvTranfer():
    w=210
    h=50
    nameWindow='UVtranferUI'
    if cmds.window(nameWindow ,ex=True):
        cmds.deleteUI(nameWindow)
    cmds.window(nameWindow, title=nameWindow+version, sizeable=False, resizeToFitChildren=True, wh=(w,h))
    ...
    b1=cmds.button(nameWindow + 'btn1',label='TODO EL GRUPO', command='PH_TRANSFERUV.UvTranferTodo()',bgc=[.3,.3,.4])
    b2=cmds.button(nameWindow + 'btn2', label="SELECCION", command='PH_TRANSFERUV.uvTranferSelect()',bgc=[1,.6,.6])
    c13= cmds.columnLayout(columnAttach=('both', 5))
    b3=cmds.button(nameWindow + 'btn3', label=" SHOW ", command='uvWindows()',bgc=[1,.6,.3])
    cb=cmds.checkBox(nameWindow + 'btn4', label=' COLLAPSE',v=True)
