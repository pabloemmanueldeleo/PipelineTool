...
def nodosGolem(*args):
	...
	for k,v in data.items():
		wRowLay['rl_'+k]=cmds.rowLayout(numberOfColumns=2,columnAttach2=['left','right'],columnWidth2=(200,500),columnAlign2=["left", "right"])
		wFilaNodo['Fila_'+k]= [cmds.checkBox(l=k),cmds.textField(width=700)]
		pop(wFilaNodo['Fila_'+k][1])
		cmds.textField(wFilaNodo['Fila_'+k][1], edit=1,text=v)
		num=num+1
		cmds.setParent('..')
...
def UI_GOLEM(*args):
	...
	wColumns['cltext_']=cmds.columnLayout(adjustableColumn=1,width=950,height=600,columnAttach=['both',0],columnOffset=['both',0])
	#Banner image
	...
	wRowLay['rlText_']=cmds.rowLayout(numberOfColumns=2,columnWidth2=(200,500),columnAlign2=['center','center'])
