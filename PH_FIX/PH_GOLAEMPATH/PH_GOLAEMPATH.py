...
#save file log
def saveFile(datos={},texto='- WARNING -'):

	name=cmds.file(q=1,sceneName=1)

	path=os.path.dirname(name)
	nameScene=os.path.basename(name)

	fileName=str(name.split('.')[0]+'_REPORT_GOLAEM.txt')
	for k,v in datos.items():
		datos[k]=v,texto
	saveJSONFile(datos,fileName)

	#writeJSONFile(datos,fileName)
...
def checkIfExistPathFile(*args):
	...
	for kp,kf in wFilaNodo.items():
		...
		if not os.path.isdir(str(path)):
			...
			report['NODO: '+kp]='RUTA: '+str(path),'ARCHIVO: '+ str(file)
...
def reporte(*args):
	global report
	conf=cmds.confirmDialog(title=wGolaem,
							message='NECESITAS QUE GENERE UN REPORTE POR TI ? \n-se creara en la misma ruta del archivo-\n',
							button=['SIPI','NOPE'], defaultButton='NOPE',messageAlign='right', cancelButton='NOPE', dismissString='NOPE',icon='question')
	if conf=='SIPI':
		saveFile(report)

def opendFolder(*args):
...
def UI_GOLEM(*args):
	...
	cmds.button(label='Abrir carpeta de archivo', c=opendFolder)
