...
def _connectToAlembic(aNode,nodoDesc,targetNode,outAtt,inAtt):
    if mc.isConnected(aNode+'.'+outAtt,nodoDesc):
        ...
        print str(aNode+'.'+outAtt) + ' no tiene conexion o ya esta conectado a otra cosa.'
...
def _makeWindow():
    ...
    mc.rowLayout(numberOfColumns=2 )
    mc.button( label='FORCE SELECT', command="PH_ALEMBICINMESH._selectToAlembic()", bgc=(1,0.4,0.3))
    mc.showWindow()
    #inicio la lista
    _initGui()
_makeWindow()