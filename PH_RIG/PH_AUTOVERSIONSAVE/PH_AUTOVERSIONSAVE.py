...
if nCompleto[:7] in carpetasPermitidas[0] or nCompleto[:7] in carpetasPermitidas[1] or nCompleto[:7] in carpetasPermitidas[2]:
    ...
    if nCompleto[1:].upper() == nCompletoLocal[1:].upper():
        mc.file(rename=discoLocal+nCompletoLocal[1:])
        mc.file(f=True, save=True, defaultExtensions=True)          
        mc.headsUpMessage ('Se ha guardado el archivo de M a Local satisfactoriamente.')
    
    # guarda incremental en local
    if nCompleto[0]== discoLocal and nCompleto[1:]!=nCompletoLocal[1:]:
        mc.file(rename=discoLocal+nCompletoLocal[1:])
        mc.file(f=True, save=True, defaultExtensions=True) 
        mc.headsUpMessage ('Se ha guardado el archivo ' + str( os.path.split(nCompletoLocal)[1])+ ' satisfactoriamente.') 
else:
    mc.headsUpMessage ("No estAs en una carpeta en la que puedas usar este script.")
