...
def crearShadowShd():
    ...
    mc.warning(msg)
    configButtom = mc.confirmDialog(title='SHADOW__SHD', message=msg, button=['Sipi', 'No'], defaultButton='Sipi',
                                    cancelButton='No', dismissString='No', icon='icon')
...
def crearAttr(name=None, defauld=None, obj=None):
    ...
    for s in sel:
        try:
            ...
            s = s
        if mc.objectType(s) == 'directionalLight' or mc.objectType(s) == 'pointLight' or mc.objectType(
                s) == 'spotLight':
            attrs = mc.listAttr(s)
            for a in attrs:
                if not str(a) == name:
...
def winLayer():
    ...
    mc.separator(height=10, style='singleDash')
    mc.button(label='SELECT HIEARCHY', c=selectH, annotation='Selecciona toda la jerarquÃ­a de la seleccion.',
              width=boxB2, height=boxB3)
    mc.separator(height=10, style='singleDash')
    mc.text(label='ATTRIBUTE SPREAD SHEET')
    mc.separator(height=10, style='singleDash')
    mc.button(label='RECEIVE SHADOW', c=reciveS,
              annotation='Prende: castsShadows,receiveShadows\nApaga: primaryVisibility', width=boxB2, height=boxB3)
    mc.button(label='PRIMARY VISIBILITY', c=primaryV,
              annotation='Prende: castsShadows,receiveShadows,primaryVisibility', width=boxB2, height=boxB3)
    mc.button(label='ADD ATTRIBUTE', c=attr, annotation='Te crea un atributo nuevo en las luces seleccionadas.',
              width=boxB2, height=boxB3)
    mc.separator(height=10, style='singleDash')
    mc.button(label='CLEANUP LAYERS', c=cleanUp,
              annotation='Trata de limpiar las conexiones no validas de los layers,siempre chequiar por las dudas.',
              width=boxB, height=boxB2, backgroundColor=naranja)
