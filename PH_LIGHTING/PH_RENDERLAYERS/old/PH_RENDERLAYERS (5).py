...
def crearAttr(name=None,defauld=None,obj=None):
	...
	for s in sel:
		if mc.objectType(s) == 'directionalLight' or mc.objectType(s)=='pointLight' or mc.objectType(s)=='spotLight':
			...
			print 'Se creo un atributo nuevo para la luz llamado ' + str(attrNew)
		else:
...
def winLayer():
	...
	mc.separator(height=10, style='singleDash')
	mc.button(label='SELECT HIEARCHY',c=selectH,annotation='Selecciona toda la jerarquÃ­a de la seleccion.',width=boxB2,height=boxB3)
	mc.separator(height=10, style='singleDash')
