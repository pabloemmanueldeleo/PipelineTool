...
def crearAttr(name=None,defauld=None,obj=None):
	...
	for s in sel:
		try:
			...
			s=s
		if mc.objectType(s) == 'directionalLight' or mc.objectType(s)=='pointLight' or mc.objectType(s)=='spotLight' or mc.objectType(s)=='areaLight':
			attrs=mc.listAttr(s)
			for a in attrs:
				if not str(a)==name:
					ojo=False
			...
			mc.warning(s+' no tengo permitido crear un atributo ahi. sorry :C')
...
def winLayer():
	...
	mc.separator(height=10, style='singleDash')
	mc.text(label='SHADERS ESENCIALES')
	mc.separator(height=10, style='singleDash')
	mc.button(label='PACK_SHADERS',c=aovshaders,annotation='Crea varios shader de arnold y aov (Asegurate que tengas el arnold como motor principal).')
	mc.text(label=' ')
	mc.rowLayout(nc=3)
