...
def crearShaderALSurface( SHD='', specS=None, nodo1R=0, nodo1G=0, nodo1B=0,*args ):
	...
	try:
		...
		pass
#crear ShaderUtility
def crearShaderUtility(SHD='',shadeMode=2,colorMode=1,nameAOV=None):
	...
	if nameAOV:
		AIWTC=SHD+'_AIWTC'
		if not cmds.objExists(AIWTC):
			createSHD('aiWriteColor',AIWTC)
		if cmds.objExists(AIWTC):
			conectar(SHD,AIWTC,'outColor', 'beauty')
			cmds.setAttr( AIWTC+'.blend',1)
			cmds.setAttr(AIWTC+'.aovName',nameAOV, type="string")
		return AIWTC
	if not cmds.objExists(SHD+'GRP'):
		createSHD('shadingEngine',SHD+'GRP')
	if cmds.objExists(SHD) and cmds.objExists(SHD+'GRP'):
...
#CREO UN AO
def crearShaderAO(SHD='', samples=1, farClip=75,nameAOV=None):
	if not cmds.objExists(SHD):
	...
	if cmds.objExists(SHD) and cmds.objExists(SHD+'GRP'):
		conectar( SHD, SHD+'GRP', 'outColor', 'surfaceShader')
		try:
			cmds.setAttr( SHD+'.samples', samples  )
			cmds.setAttr( SHD+'.farClip', farClip )
		except:
			pass
	if nameAOV:
		AIWTC=SHD+'_AIWTC'
...
def packShaders():
	...
	id=crearShaderUtility( 'ID__SHD'   , 2 , 21 ,'OBJECT_ID' )
	normal=crearShaderUtility( 'NORMAL__SHD',2 , 3 ,'NORMAL' )
	#SHADER DESEADO AO
	ao=crearShaderAO( 'AO__SHD', 6, 75 ,'OCCLUSION' )

	#CONEXION AOV CON SHADER
	conectarAOVconShader('aiAOV_OBJECT_ID','OBJECT_ID',id)
	conectarAOVconShader('aiAOV_OCCLUSION','OCCLUSION',ao)
	conectarAOVconShader('aiAOV_NORMAL','NORMAL',normal)
