...
def add_light(kind):
    ...
    if descripcionDelRootOK=='OK':
        ...
        if rootAts:
            for at in rootAts:
                if str(at) != 'visibility':
                    cmds.setAttr (root+"."+at,lock=1,k=0,channelBox=0)
                else:
                    cmds.setAttr (root+"."+at,k=1,channelBox=1)
    else:
        descripcionDelRootOK='YA HAY ROOT'
...
def duplicarLuz(*args):
    ...
    for luz in lucesSeleccionadas:
        if luz.split("_")[-2].isdigit()==True:
            nombreLuzTRF = luz[:-8] + '_' + str(int(luz[-7:-5])+1).zfill(3) + luz[-5:]
            while cmds.objExists (nombreLuzTRF)==1:
                nombreLuzTRF = luz[:-8]+ '_' + str(int(nombreLuzTRF[-8:-5])+1).zfill(3)  + luz[-5:]
        else:
            nombreLuzTRF = luz
...
def tglTodos():
    ...
    for boton in botonesFiltros:
        cmds.iconTextButton ("b_filtro"+boton,e=1,i1=RUTAICONOS+boton+"_"+str(int(not(imagenEstado)))+".png")
    filtradoRefresh()
...
def filtrado(luzTipo):
    ...
    if int(imagen[-5])==1:
        cmds.iconTextButton ('b_filtro'+luzTipo , e=1 , i1 = RUTAICONOS+luzTipo+"_0.png" )
    if int(imagen[-5]) == 0 :
        cmds.iconTextButton ('b_filtro'+luzTipo , e=1 , i1 = RUTAICONOS+luzTipo+"_1.png" )
    filtradoRefresh()
...
def refreshui(refrescarPor="",*args):
    ...
    if lucesSeleccionadas != None:
        camposAtributos       = { "aiUseColorTemperature":0 , "on_off":0 , "aiExposure":0 , "aiRadius":0, "aiColorTemperature":0, "color":0, "intensity":0, "aiAngle":0, "aiSamples":0,"emitDiffuse":0,"emitSpecular":0,"coneAngle":0 ,"penumbraAngle":0 ,"dropoff":0 , "aiShadowDensity":0 }
        camposAtributosV      = { "aiUseColorTemperature":0 , "on_off":0 , "aiExposure":0 , "aiRadius":0, "aiColorTemperature":0, "color":None, "intensity":0, "aiAngle":0, "aiSamples":0,"emitDiffuse":0,"emitSpecular":0 ,"coneAngle":0  ,"penumbraAngle":0 ,"dropoff":0 , "aiShadowDensity":0 }
        camposAtributosDif    = { "aiUseColorTemperature":0 , "on_off":0 , "aiExposure":0 , "aiRadius":0, "aiColorTemperature":0, "color":0, "intensity":0, "aiAngle":0, "aiSamples":0,"emitDiffuse":0,"emitSpecular":0 , "coneAngle":0  ,"penumbraAngle":0 ,"dropoff":0 , "aiShadowDensity":0 }
        # hide / unhide
        ...
        checkearAtr ( "on_off" , "on_off" ,camposAtributosDif,camposAtributosV )
        checkearAtr ( "aiUseColorTemperature" , "aiColorTemperature" , camposAtributosDif , camposAtributosV  )
        dropListas()
        boldOblique()
        enableDropParent()
        enableTempColor ()
...
def checkearAtr( at , campo,camposAtributosDif,camposAtributosV, *args):
    if camposAtributosDif [ at ] ==0 : #tienen valores iguales
        try:
            cmds.checkBox ( campo , e=1 , en=1  , v=camposAtributosV [ at ] ,  bgc=[0.3,0.3,0.3] )
        except:
            pass
    elif camposAtributosDif [ at ] !=0 :#tienen valores diferentes
        try:
            cmds.checkBox ( campo , e=1 ,  en=1 , bgc=[0.863,0.808,0.529])
        except:
            pass
...
def construirScrollsConBotones( emparentarA='') :
    ...
    cmds.iconTextButton  (l ='LUZ',ann='LUZ',style='textOnly',image1=RUTAICONOS+'LSPT_1.png', c = ordenarLuz     ,p=r_00 ,bgc=(0.27,0.27,0.27) ,font= "plainLabelFont")
    cmds.iconTextButton (l ='TIPO',ann='TIPO',style='textOnly',image1=RUTAICONOS+'LPNT_1.png', c = ordenarTipo2    ,p=r_00 ,bgc=(0.27,0.27,0.27) ,font= "plainLabelFont")
    cmds.iconTextButton  (l ='GRUPO',ann='GRUPO',style='textOnly',image1=RUTAICONOS+'LARE_1.png', c = ordenarGrupo ,p=r_00 ,bgc=(0.27,0.27,0.27) ,font= "plainLabelFont")

    b_isolate = cmds.iconTextButton  ('b_isolate',label='A',ann='AISLAR',en=1, style='iconOnly' ,image1=RUTAICONOS+'PH_LUCES_ISOLATE_OFF.png' , c = isolateLuz ,width=15,height=25,p=r_00 )
    if 'isolate' in globals():
        if isolate == True:
            cmds.iconTextButton  ('b_isolate' , e = 1 ,  image1 = RUTAICONOS+'PH_LUCES_ISOLATE.png' )
    ...
    try:
        ...
        if len(textoBuscador)!=0:
            if buscarPor_Grupo_Luz=='luz':
                if str(textoBuscador).isdigit() ==1:
    ...
    if isolate == False:
        ...
        for luz in luces:
            ...
            if dicEstadoLoad [ luzTRF ]:
                ...
                cmds.hide ( luz )
    if 'isolate' not in globals():
            cmds.iconTextButton  ('b_isolate' , e = 1 ,  image1 = RUTAICONOS+'PH_LUCES_ISOLATE_OFF.png' )
    else:
        if isolate == True:
            cmds.iconTextButton  ('b_isolate' , e = 1 ,  image1 = RUTAICONOS+'PH_LUCES_ISOLATE.png' )
        else:
...
def lightListPanel(*args):
    ...
    b_filtroSpot = cmds.iconTextButton  ('b_filtroLSPT',ann='SPOT',style='iconOnly',image1=RUTAICONOS+'LSPT_1.png', c = partial(filtrado,"LSPT") ,width=25,height=25,p=row_0, dcc=partial(add_light,"spot") ,bgc=(0.2,0.2,0.2),font= "fixedWidthFont")
    b_filtroPoint = cmds.iconTextButton ('b_filtroLPNT',ann='POINT',style='iconOnly',image1=RUTAICONOS+'LPNT_1.png', c = partial(filtrado,"LPNT") , width=25,height=25,p=row_0, dcc=partial(add_light,"point") ,bgc=(0.2,0.2,0.2),font= "fixedWidthFont")
    b_filtroArea = cmds.iconTextButton  ('b_filtroLARE',ann='AREA',style='iconOnly',image1=RUTAICONOS+'LARE_1.png', c = partial(filtrado,"LARE") ,width=25,height=25,p=row_0, dcc=partial(add_light,"area") ,bgc=(0.2,0.2,0.2),font= "fixedWidthFont")
    b_filtroDir = cmds.iconTextButton   ('b_filtroLDIR',ann='DIRECTIONAL',style='iconOnly',image1=RUTAICONOS+'LDIR_1.png', c = partial(filtrado,"LDIR") ,width=25,height=25,p=row_0, dcc=partial(add_light,"dir") ,bgc=(0.2,0.2,0.2),font= "fixedWidthFont")
    b_all = cmds.iconTextButton         ('b_filtrotodos',ann='TOGGLE FILTROS TODOS',style='iconOnly', image1=RUTAICONOS+'TODAS_1.png',width=25,height=25,p=row_0,c=tglTodos )
    cmds.separator ( hr=0 , w=10 , p = row_0 )
    grupo_crear = cmds.iconTextButton   (style='iconOnly', image1 = RUTAICONOS+'PH_LUCES_CARPETA.png' ,width=25,height=25, c=crearGrupo , ann="CREA UN GRUPO DE LUZ CON EL NOMBRE ESPECIFICADO",  p = row_0 )
    grupo_renombrar = cmds.iconTextButton   (style='iconOnly', image1 = RUTAICONOS+'PH_LUCES_CARPETA_RENOMBRAR.png',width=25,height=25, c=renombrarGrupo , ann="RENOMBRA EL GRUPO ELEGIDO EN EL DROP.\nDESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.",  p = row_0 )
    grupo_borrar = cmds.iconTextButton   (style='iconOnly', image1 = RUTAICONOS+'PH_LUCES_CARPETA_BORRAR.png' ,width=25,height=25, c=borrarGrupo , ann="BORRA EL GRUPO ELEGIDO EN EL DROP.\nDESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.",  p = row_0 )
    ...
    tempColor = cmds.floatField('aiColorTemperature' ,w=40 ,minValue=0, maxValue = 15000 ,  bgc=(1,0.2,0.2) , en=0 , pre=0 ,h=15 ,cc = setTempColor, p=row_filtros )
    ...
    b_sss = cmds.iconTextButton  ('b_sss',l='SSS',style='textOnly',image1=RUTAICONOS+'PH_LUCES_SSS.png', c = sss ,width=anchoBotones,height=25,p=row_6 ,bgc=(0.2,0.2,0.2))
    b_disp = cmds.iconTextButton  ('b_disp',l='DISPLACEMENT',style='textOnly',image1=RUTAICONOS+'PH_LUCES_DISP.png', c = disp ,width=anchoBotones,height=25,p=row_6 ,bgc=(0.2,0.2,0.2))
    b_sub = cmds.iconTextButton  ('b_sub',l='SUBDIVISION',style='textOnly',image1=RUTAICONOS+'PH_LUCES_DISP.png', c = sub ,width=anchoBotones,height=25,p=row_6 ,bgc=(0.4,0.4,0.4))
    b_flush = cmds.iconTextButton  ('b_flush',l='FLUSH CACHE',style='textOnly',image1=RUTAICONOS+'PH_LUCES_DISP.png', c = "cmds.arnoldFlushCache(flushall=True)\ncmds.warning('FLUSH')" ,width=anchoBotones,height=25,p=row_6 ,bgc=(0.4,0.4,0.4))
    ...
    b_domoOnOff = cmds.iconTextButton  ('b_domoOnOff',l='DOMO',style='textOnly',image1=RUTAICONOS+'PH_LUCES_DISP.png', c = domoOnOff,width=anchoBotones,height=25,p=row_6 ,bgc=(0.4,0.4,0.4))
    ...
    #    cmds.warning('Problemas al iniciar manager de luces.\nLlamar a sus creadores porfavor.')
