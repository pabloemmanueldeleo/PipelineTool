	...
	if rootDeEscena != "" or len(listaDeRootsNoReferenciados)==1:
		while pideNombreGrupo=='OK' and arrancaConNumero==1:
			...
			if cmds.promptDialog(query=True, text=True)!="" and pideNombreGrupo != "CANCELAR":
				if pideNombreGrupo == 'OK' and str(cmds.promptDialog(query=True, text=True))[0].isdigit()==False and not cmds.objExists( cmds.promptDialog(query=True, text=True).upper()+"_LGRP" ):
					...
					cmds.warning ( " - - - CANCELADO O NOMBRE INVALIDO - - - " )
					return ""
			else:
...
def renombrarGrupo(*args):
	global grupoVG
	print "\n\n klsdklklsda\n\n"
	if  len (cmds.ls ("*_LGRP", rn=0)) > 1:
		print "len " + len (cmds.ls ("*_LGRP", rn=0))
...
def lightListPanel():
	...
	dockLuces = cmds.dockControl( 'dockLucesUI' , label= "PH_LUCES! v1.7 - COMPATIBLE CON ARNOLD -",area='right', content=win, allowedArea=allowedAreas )
