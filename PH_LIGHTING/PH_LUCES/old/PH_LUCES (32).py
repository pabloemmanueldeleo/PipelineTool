...
def visibilidadDeRuta(*args):
    seleccionLista1 = cmds.textScrollList ('listaLuces1' , q=1 , si=1)[0]+"SH"
    seleccionDeLaEscena = cmds.ls (sl=1)
    global filtro
    if seleccionLista1:
    ...
    for i,v in enumerate(parents):
        if i!=0 and i!=len(parents)-1:
            acumulador +=   parents [i] + "|"
        elif i!=0 and i==len(parents)-1:
            acumulador +=   parents [i]
        else:
            acumulador =  parents [i] + "|"
        parentsFullPath.append (acumulador)
    outliner = cmds.getPanel ( type='outlinerPanel')[0]
    cmds.OutlinerWindow ()
    filtro = cmds.itemFilter  ( byType = ('spotLight','areaLight','pointLight','directionalLight')  )
    cmds.select(all=1,hi=1)
    mel.eval("doHideInOutliner 1;")
    cmds.select (parentsFullPath)
    mel.eval("doHideInOutliner 0;")
    cmds.outlinerEditor ( outliner ,e=1, ignoreHiddenAttribute=0 , showSelected=1 , setFilter = filtro )
    gruposDeLucesParaEsconder = cmds.listRelatives ( parentsFullPath[0] , c=1 )
    gruposDeLucesParaEsconder.remove(parents[1])
    cmds.select ( gruposDeLucesParaEsconder )
    mel.eval("doHideInOutliner 1;")
    cmds.select (seleccionLista1)
    cmds.outlinerEditor (outliner ,e=1,  showSelected=1 )
    cmds.select(seleccionDeLaEscena)

def cierraOutliner(*args):####################################################################################################
    global jobCierreOut
    global filtro
    seleccionDeLaEscena = cmds.ls (sl=1)
    outliner = cmds.getPanel ( type='outlinerPanel')[0]
    parentOutliner = (cmds.outlinerEditor ( outliner ,q =1, p=1 )).split("|")[0]
    cmds.delete ( filtro )
    cmds.select(all=1,hi=1)
    mel.eval('doHideInOutliner 0;')
    cmds.select(all=1,hi=1)
    cmds.outlinerEditor ( outliner,e=1,showSelected=0,showAttributes=0,autoExpand=0  )
    cmds.select(seleccionDeLaEscena)
...
def dropListas():############################################################################################################################################
	...
	cmds.menuItem( divider=1, dividerLabel= " " , p=pop)
	cmds.menuItem(l="OUTLINER" , ann="VOLVER A VER EL OUTLINER COMO TODOS ESTAMOS ACOSTUMBRADOS A VERLO.",  c=cierraOutliner, p=pop)
...
def lightListPanel():
    try:
    	...
    	b_domoOnOff = cmds.iconTextButton  ('b_domoOnOff',l='DOMO ON',style='textOnly',image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png', c = domoOnOff,width=90,height=25,p=row_6 ,bgc=(0.4,0.4,0.4))
...
def domoOnOff(*args):
    if cmds.ls (type='aiSkyDomeLight'):
        ...
        v = cmds.getAttr (domos[0]+".visibility") # estado de la visibilidad
        for d in domos:
            cmds.setAttr ( d+".visibility" , not v  ) # seteo visibilidad invertida
...
def refreshOverride (*args):
    ...
    if cmds.ls (type='aiSkyDomeLight'):
        ...
        for d in domos:
            if cmds.getAttr (d +".visibility")!=v:
                dif = 1 # hay diferentes valores
        if not dif :
            if v:
                cmds.iconTextButton  ('b_domoOnOff' , e=1 , bgc=verde )
            else:
                cmds.iconTextButton  ('b_domoOnOff' , e=1 , bgc=rojo )
        else:
