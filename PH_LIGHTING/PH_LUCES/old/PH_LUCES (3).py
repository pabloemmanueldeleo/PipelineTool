...
def add_light(kind=""):
	...
	winCrearLuz='crearLuzV'
	if cmds.window(winCrearLuz,exists=True):
		cmds.deleteUI (winCrearLuz)
	...
	nombreLuzNueva=cmds.textField ('nombreLuzCreacion' , pht='NOMBRE' , p=col_ventanaCrearLuz ,changeCommand= 'crearLuz(kind)' )
	...
	cmds.showWindow (winCrearLuz)

def crearLuz(kind="",*args):
	global nombreLuzNueva
	global winCrearLuz
	nLuz=cmds.textField('nombreLuzCreacion',q=1,text=1)
	nFuncion= cmds.radioCollection('coleccionTodo',q=1, select=1)
	nombreLuz = nLuz+"_"+nFuncion+"_"
	cmds.deleteUI (winCrearLuz)

	if nLuz=="" :
		nombreLuz = 'RENAMEMEPLEASE'+"_"+nFuncion+"_"
	if kind == 'spot':
...
def ordenarDatos(ordenarPor='luz' , filtrado=''):
	if filtrado=='':
	...
	# ORDENA POR TIPO
		...
		print lucesOrdenadas_,tiposOrdenados_,gruposOrdenados_
	##########
	# ORDENA POR GRUPO
...
def construirScrollsConBotones( emparentarA='') :
	...
	datosOrdenados = ordenarDatos(ordenarPor='luz' , filtrado='')
	cmds.textScrollList ('listaLuces1' , e=1 ,allowMultiSelection=1, a=datosOrdenados[0] , sc=partial(refreshui,'luz') , dcc=seleccionarLuces ,  enable=1 ,h = (len(datosOrdenados[0])+2)*50)
	cmds.textScrollList ('listaLuces2' , e=1 ,allowMultiSelection=1, a=datosOrdenados[1] , sc=partial(refreshui,'tipo') , dcc=seleccionarLuces  , enable=1 , h = (len(datosOrdenados[0])+2)*50 )
	cmds.textScrollList ('listaLuces3' , e=1 ,allowMultiSelection=1, a=datosOrdenados[2] , sc=partial(refreshui,'grupo') , dcc=seleccionarLuces  , enable=1 ,h = (len(datosOrdenados[0])+2)*50 )
...
def lightListPanel():
	...
	b_filtroArea = cmds.iconTextButton  ('b_filtroLARE',ann='AREA',vis=0,style='iconOnly',image1='M:\PH_SCRIPTS\ICONS\LARE_1.png',hi='M:\PH_SCRIPTS\ICONS\LARE_1.png', c = partial(filtrado,"LARE") ,width=25,height=25,p=row_0, dcc=partial(add_light,"area") ,bgc=(0.2,0.2,0.2),font= "fixedWidthFont")
	...
	buscador = cmds.textField ( 'buscador' , p = row_0 , changeCommand = buscar , pht="Buscador" )
	...
	cmds.colorSliderGrp( 'aiColorTemperature' , en=0, cw2 = (20, 0), vis=0 , changeCommand = setSamp , p=row_2)
