...
def add_light(kind):
    ...
    if (not cmds.ls('ROOT_*_LGRP')):
        ...
        cmds.warning(" - - - - CANCELADO POR EL USUARIO - - - - ")
    ...
    if descripcionDelRootOK == 'OK' and len(grupos_LGRP) == 1:
        cmds.confirmDialog(title='AVISO', message='NO HAY GRUPOS DE LUCES EN LA ESCENA. CREEMOS UNO.', button=['OKI'],
                           defaultButton='OKI')
        primerGrupoCreado = crearGrupo()
    roots_LGRP = cmds.ls('ROOT_*_LGRP')
    if (len(grupos_LGRP) > 1 and len(roots_LGRP) == 1) or descripcionDelRootOK == 'OK':
...
def crearLuz(kind="", primerGrupoCreado='', nombre=''):
    if nombre == "" or nombre == False:
		nombreLuz = cmds.textField('nombreLuzCreacion', q=1, text=1) + "_" + cmds.radioCollection('coleccionTodo', q=1,
                                                                                                  select=1) + "_"
        grupoDeLaLuzCreada = cmds.optionMenu('oM_grupoVCreacion|OptionMenu', q=1, value=1)
        if nombreLuz == "_" + cmds.radioCollection('coleccionTodo', q=1, select=1) + "_" or nombreLuz[
            0] in "0123456789":  # si no puso ningun nombre
...
def tglTodos():
    ...
    for boton in botonesFiltros:
        cmds.iconTextButton("b_filtro" + boton, e=1,
                            i1="M:\PH_SCRIPTS\ICONS\\" + boton + "_" + str(int(not (imagenEstado))) + ".png")
    filtradoRefresh()
...
def refreshui(refrescarPor="", *args):
    ...
    if lucesSeleccionadas != None:
        camposAtributos = {"on_off": 0, "aiExposure": 0, "aiRadius": 0, "aiColorTemperature": 0, "color": 0,
                           "intensity": 0, "aiAngle": 0, "aiSamples": 0, "emitDiffuse": 0, "emitSpecular": 0,
                           "coneAngle": 0, "penumbraAngle": 0, "dropoff": 0, "aiShadowDensity": 0}
        camposAtributosV = {"on_off": 0, "aiExposure": 0, "aiRadius": 0, "aiColorTemperature": 0, "color": None,
                            "intensity": 0, "aiAngle": 0, "aiSamples": 0, "emitDiffuse": 0, "emitSpecular": 0,
                            "coneAngle": 0, "penumbraAngle": 0, "dropoff": 0, "aiShadowDensity": 0}
        camposAtributosDif = {"on_off": 0, "aiExposure": 0, "aiRadius": 0, "aiColorTemperature": 0, "color": 0,
                              "intensity": 0, "aiAngle": 0, "aiSamples": 0, "emitDiffuse": 0, "emitSpecular": 0,
							  "coneAngle": 0, "penumbraAngle": 0, "dropoff": 0, "aiShadowDensity": 0}
        # hide / unhide
        ...
        for campo in camposAtributos.keys():
            if camposAtributosDif[campo] == 0 and len(lucesSeleccionadas) == camposAtributos[
                campo]:  # tienen valores iguales
				try:
                ...
                try:
                    ...
                    pass
            elif camposAtributosDif[campo] != 0 and len(lucesSeleccionadas) == camposAtributos[
                campo]:  # tienen valores diferentes
                try:
...
def boldOblique():
    ...
    if luces != None:
        for luz in luces:
            ...
            cmds.textScrollList("listaLuces2", e=1, da=1)
            cmds.textScrollList("listaLuces3", e=1, da=1)
            if cmds.getAttr(luz + ".visibility") and cmds.referenceQuery(luz,
                                                                         isNodeReferenced=1) == True:  # referenciada y activa
            ...
            elif cmds.getAttr(luz + ".visibility") == False and cmds.referenceQuery(luz,
                                                                                    isNodeReferenced=1) == True:  # referenciada e inactiva
                cmds.textScrollList("listaLuces1", e=1, si=luz)
                index = cmds.textScrollList("listaLuces1", q=1, sii=1)[0]
                ...
                cmds.textScrollList("listaLuces3", e=1, lf=[index, "tinyBoldLabelFont"])
                                                                                    ...
                                                                                    isNodeReferenced=1) == False:  # referenciada e inactiva
                cmds.textScrollList("listaLuces1", e=1, si=luz)
                index = cmds.textScrollList("listaLuces1", q=1, sii=1)[0]
            ...
            elif cmds.getAttr(luz + ".visibility") == True and cmds.referenceQuery(luz,
																				   isNodeReferenced=1) == False:  # referenciada e inactiva
                cmds.textScrollList("listaLuces1", e=1, si=luz)
                index = cmds.textScrollList("listaLuces1", q=1, sii=1)[0]
...
def enableDropParent():
    ...
    if cmds.textScrollList('listaLuces1', q=1, si=1) == None:
        ...
        cmds.optionMenuGrp(grupoCParent, e=1, en=0,
                           ann="DESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.")
...
def enableTempColor(
    ...
    if lucesSeleccionadas != None:
        ...
        if len(tienenTempColorOn) == len(lucesSeleccionadas):
            ...
            cmds.floatField("aiColorTemperature", e=1, en=1, bgc=[0.8, 0.8, 0.8])
            ...
            cmds.floatField("aiColorTemperature", e=1, en=1, bgc=[0.4, 0.4, 0.4])
...
def maxmin():
    ...
    camposAtributosMin = {"aiExposure": 0, "aiRadius": 0, "intensity": 0, "aiAngle": 0, "aiSamples": 0,
                          "aiColorTemperature": 0, "aiShadowDensity": 0}
    ...
    if len(lucesSeleccionadas) > 1:
        for campo in camposAtributosMin.keys():
            try:
                cmds.floatField(campo, e=1, ann="MIN: " + camposAtributosMin[campo][0] + "  " + str(
                    camposAtributosMin[campo][1]) + "\n" + "MAX: " + camposAtributosMax[campo][0] + "  " + str(
                    camposAtributosMax[campo][1]) + "\n" + "PROMEDIO: " + str(
                    (camposAtributosMax[campo][1] - camposAtributosMin[campo][1]) / 2 + camposAtributosMin[campo][1]))
            except:
                pass
    else:
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.intensity',
                                 cmds.getAttr(luz + 'SH.intensity') + cmds.floatField('intensity', q=1, v=1))
            except:
                pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiExposure',
                                 cmds.getAttr(luz + 'SH.aiExposure') + cmds.floatField('aiExposure', q=1, v=1))
            except:
                pass
        cmds.textScrollList('listaLuces1', e=1, si=lucesSeleccionadas)
    except:
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiRadius',
                                 cmds.getAttr(luz + 'SH.aiRadius') + cmds.floatField('aiRadius', q=1, v=1))
            except:
                pass
    except:
        pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiAngle',
                                 cmds.getAttr(luz + 'SH.aiAngle') + cmds.floatField('aiAngle', q=1, v=1))
            except:
                pass
    except:
        pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiShadowDensity',
                                 cmds.getAttr(luz + 'SH.aiShadowDensity') + cmds.floatField('aiShadowDensity', q=1,
                                                                                            v=1))
            except:
                pass
    except:
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiSamples',
                                 cmds.getAttr(luz + 'SH.aiSamples') + cmds.floatField('aiSamples', q=1, v=1))
            except:
                pass
    except:
        pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.aiColorTemperature',
								 cmds.getAttr(luz + 'SH.aiColorTemperature') + cmds.floatField('aiColorTemperature',
                                                                                               q=1, v=1))
            except:
                pass
    except:
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.coneAngle',
                                 cmds.getAttr(luz + 'SH.coneAngle') + cmds.floatField('coneAngle', q=1, v=1))
            except:
                pass
    except:
        pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.penumbraAngle',
                                 cmds.getAttr(luz + 'SH.penumbraAngle') + cmds.floatField('penumbraAngle', q=1, v=1))
            except:
                pass
    except:
        pass
        ...
        for luz in lucesSeleccionadas:
            try:
                if cmds.radioCollection('radioAbsRel', q=1, sl=1) == 'A':
                    ...
                    cmds.setAttr(luz + 'SH.dropoff',
                                 cmds.getAttr(luz + 'SH.dropoff') + cmds.floatField('dropoff', q=1, v=1))
            except:
                pass
    except:
        pass
...
def ordenarLuz(seleccionar=1):
    ...
    if len(lucesTodas) != 0:
        ...
        cmds.textScrollList('listaLuces1', e=1, a=datosOrdenados[0], numberOfRows=len(datosOrdenados[0]) + 2,
                            si=seleccionar)
...
def cambiarGrupo(grupoParaMoverA=""):
    ...
    if cmds.textScrollList('listaLuces1', q=1, si=1) != None:
        ...
        for l in seleccionLista1:
            try:
                cmds.textScrollList('listaLuces1', e=1, si=l)
            except:
                pass
...
def crearGrupo(*args):
    ...
    if rootDeEscena != "" or len(listaDeRootsNoReferenciados) == 1:
        while pideNombreGrupo == 'OK' and arrancaConNumero == 1:
            pideNombreGrupo = cmds.promptDialog(title='NOMBRAR', message='NOMBRE DE GRUPO:', button=['OK', 'CANCELAR'],
                                                defaultButton='OK', cancelButton='CANCELAR', dismissString='Cancel')
...
def construirScrollsConBotones(emparentarA=''):
    ...
    r_00 = cmds.rowLayout('r_00', nc=5, p='row1', h=30, cw5=[50, 250, 85, 100, 25],
                          ct5=["both", "both", "both", "both", "both"])
    cmds.iconTextButton(ann='REFRESCAR GUI', style='iconOnly', image1='refresh.png', c=refrescar, width=15, height=20,
                        p=r_00)
    ...
    b_isolate = cmds.iconTextButton('b_isolate', ann='AISLAR LUZ', en=0, style='iconOnly',
                                    image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_ISOLATE.png', c=aislarLuz, width=15, height=25,
                                    p=r_00)
    ...
    cmds.textScrollList('listaLuces2', w=100, allowMultiSelection=1, p=c_02, bgc=[0.24, 0.24, 0.24], dcc=tipoSel,
                        deleteKeyCommand=borrarSeleccion)
    cmds.textScrollList('listaLuces3', w=100, allowMultiSelection=1, p=c_03, bgc=[0.24, 0.24, 0.24], dcc=grpSel,
                        deleteKeyCommand=borrarSeleccion)
    datosOrdenados = ordenarDatos(ordenarPor='luz', filtrado='')
    cmds.textScrollList('listaLuces1', e=1, allowMultiSelection=1, a=datosOrdenados[0], sc=partial(refreshui, 'luz'),
                        enable=1, h=(len(datosOrdenados[0]) + 2) * 100)
    cmds.textScrollList('listaLuces2', e=1, allowMultiSelection=1, a=datosOrdenados[1], sc=partial(refreshui, 'tipo'),
                        enable=1, h=(len(datosOrdenados[0]) + 2) * 100)
    cmds.textScrollList('listaLuces3', e=1, allowMultiSelection=1, a=datosOrdenados[2], sc=partial(refreshui, 'grupo'),
                        enable=1, h=(len(datosOrdenados[0]) + 2) * 100)
        ...
        if len(textoBuscador) != 0:
            if buscarPor_Grupo_Luz == 'luz':
                if str(textoBuscador[0]).isdigit() == 1:
                    ...
                    recolectorBusqueda = cmds.ls(textoBuscador + "*", "*" + textoBuscador, "*" + textoBuscador + "*",
												 textoBuscador, textoBuscador.upper() + "*",
                                                 "*" + textoBuscador.upper(), "*" + textoBuscador.upper() + "*",
												 textoBuscador.upper(), type='light')
                ...
                if str(textoBuscador[0]).isdigit() == 1:
                    ...
                    gruposRecolectados = cmds.ls(textoBuscador + "*", "*" + textoBuscador, "*" + textoBuscador + "*",
                                                 textoBuscador, textoBuscador.upper() + "*",
                                                 "*" + textoBuscador.upper(), "*" + textoBuscador.upper() + "*",
                                                 textoBuscador.upper(), type='transform')
                    gruposRecolectados = list(set(gruposRecolectados))
...
def cambiarGrupoParent(*args):
    ...
    if cmds.optionMenuGrp(grupoVG, q=1, v=1) != cmds.optionMenuGrp(grupoCParent, q=1, v=1):
        try:
            cmds.parent(cmds.optionMenuGrp(grupoVG, q=1, v=1) + "_LGRP",
                        cmds.optionMenuGrp(grupoCParent, q=1, v=1) + "_LGRP")
        except:
            pass
    refreshui()
...
def borrarGrupo(*args):
    ...
    if cmds.optionMenuGrp(grupoVG, q=1, v=1) != "":
        confirma = cmds.confirmDialog(title='BORRAR GRUPO',
                                      message='BORRAR ' + cmds.optionMenuGrp(grupoVG, q=1, v=1) + '?',
                                      button=['SI', 'NO'], defaultButton='SI', cancelButton='NO', dismissString='NO')
        if confirma == "SI":
...
def sss(*args):
    ...
    if deactivateSSS == True:
        deactivateSSS = False
        cmds.iconTextButton('b_sss', e=1, bgc=[0.5, 1, 0.5])
        cmds.setAttr('defaultArnoldRenderOptions.ignoreSss', 0)
    else:
        deactivateSSS = True
        cmds.iconTextButton('b_sss', e=1, bgc=[1, 0.5, 0.5])
...
def disp(*args):
    ...
    if deactivateDisplacement == True:
        deactivateDisplacement = False
        cmds.iconTextButton('b_disp', e=1, bgc=[0.5, 1, 0.5])
        cmds.setAttr('defaultArnoldRenderOptions.ignoreDisplacement', 0)
    else:
        deactivateDisplacement = True
        cmds.iconTextButton('b_disp', e=1, bgc=[1, 0.5, 0.5])
...
def sub(*args):
    ...
    if deactivateSUB == True:
        deactivateSUB = False
        cmds.iconTextButton('b_sub', e=1, bgc=[0.5, 1, 0.5])
        cmds.setAttr('defaultArnoldRenderOptions.ignoreSubdivision', 0)
    else:
        deactivateSUB = True
        cmds.iconTextButton('b_sub', e=1, bgc=[1, 0.5, 0.5])
...
def crearPopTempColor(parent_, *args):
    ...
    cmds.menuItem(l="USAR TC ACTUAL", p=popTempColor, c=usarTcACtual)
...
def lightListPanel():
    ...
    global buscarPor_Grupo_Luz
    global myLights
    global myLightsOnShapes
    global myLightsOn
    global isolate
    global vecesEjecutado
    try:
        import UTILITIES
        UTILITIES.arnoldON()
        refreshOverride()
    ...
    win = cmds.window('winMLuces', title="PH_LUCES! v1.6 - COMPATIBLE CON ARNOLD -", menuBar=0, w=100, s=1, height=100)
    ...
    b_sel = cmds.iconTextButton('b_sel', ann='(.)SELECCIONAR - (..)DESELECCIONAR', style='iconOnly',
								image1='selectObject.png', c=seleccionarLuces, dcc=deseleccionar, width=25, height=25,
								p=row_0, bgc=(0.4, 0.4, 0.4), font="fixedWidthFont")
	cmds.separator(w=20, p=row_0, st="none")
    b_filtroSpot = cmds.iconTextButton('b_filtroLSPT', ann='SPOT', style='iconOnly',
                                       image1='M:\PH_SCRIPTS\ICONS\LSPT_1.png', c=partial(filtrado, "LSPT"), width=25,
                                       height=25, p=row_0, dcc=partial(add_light, "spot"), bgc=(0.2, 0.2, 0.2),
                                       font="fixedWidthFont")
    b_filtroPoint = cmds.iconTextButton('b_filtroLPNT', ann='POINT', style='iconOnly',
                                        image1='M:\PH_SCRIPTS\ICONS\LPNT_1.png', c=partial(filtrado, "LPNT"), width=25,
                                        height=25, p=row_0, dcc=partial(add_light, "point"), bgc=(0.2, 0.2, 0.2),
                                        font="fixedWidthFont")
    b_filtroArea = cmds.iconTextButton('b_filtroLARE', ann='AREA', style='iconOnly',
                                       image1='M:\PH_SCRIPTS\ICONS\LARE_1.png', c=partial(filtrado, "LARE"), width=25,
                                       height=25, p=row_0, dcc=partial(add_light, "area"), bgc=(0.2, 0.2, 0.2),
                                       font="fixedWidthFont")
    b_filtroDir = cmds.iconTextButton('b_filtroLDIR', ann='DIRECTIONAL', style='iconOnly',
                                      image1='M:\PH_SCRIPTS\ICONS\LDIR_1.png', c=partial(filtrado, "LDIR"), width=25,
                                      height=25, p=row_0, dcc=partial(add_light, "dir"), bgc=(0.2, 0.2, 0.2),
                                      font="fixedWidthFont")
    b_all = cmds.iconTextButton('b_filtrotodos', ann='TOGGLE FILTROS TODOS', style='iconOnly',
								image1='M:\PH_SCRIPTS\ICONS\TODAS_1.png', width=25, height=25, p=row_0, c=tglTodos)
	cmds.separator(hr=0, w=10, p=row_0)
	grupo_crear = cmds.iconTextButton(style='iconOnly', image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_CARPETA.png', width=25,
                                      height=25, c=crearGrupo, ann="CREA UN GRUPO DE LUZ CON EL NOMBRE ESPECIFICADO",
                                      p=row_0)
    grupo_renombrar = cmds.iconTextButton(style='iconOnly', image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_CARPETA_RENOMBRAR.png',
                                          width=25, height=25, c=renombrarGrupo,
                                          ann="RENOMBRA EL GRUPO ELEGIDO EN EL DROP.\nDESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.",
                                          p=row_0)
    grupo_borrar = cmds.iconTextButton(style='iconOnly', image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_CARPETA_BORRAR.png',
                                       width=25, height=25, c=borrarGrupo,
                                       ann="BORRA EL GRUPO ELEGIDO EN EL DROP.\nDESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.",
                                       p=row_0)
    grupoVG = cmds.optionMenuGrp('oM_grupo', bgc=[0.1, 0.1, 0.1], w=70, changeCommand=cambiarGrupo, p=row_0)
    grupoCParent = cmds.optionMenuGrp('oM_grupoParent', en=0,
                                      ann="DESELECCIONA LAS LUCES DE LA LISTA PARA HABILITAR ESTA OPCION.",
                                      bgc=[0.27, 0.27, 0.27], w=70, changeCommand=cambiarGrupoParent, p=row_0)
    ...
    scroll = cmds.scrollLayout('scroll', parent=win, childResizable=1, width=520, h=550)
    ...
    frame_1 = cmds.frameLayout('frame_1', h=120,
                               expandCommand="cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)+102 )",
                               collapseCommand="cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)-102 )\ncmds.window('winMLuces', e=1 , h=cmds.window('winMLuces', q=1 , height= 1)-80 )",
                               label='ATRIBUTOS', borderStyle='in', collapsable=1, p=win, w=520)
    rootDeLaEscena = ""
    roots = cmds.ls('ROOT_*_LGRP', r=1)
    listaDeRootsNoReferenciados = []
    for root in roots:
    ...
    cmds.radioButton('A', label='ABSOLUTO', sl=1, ann="ABSOLUTO", cl='radioAbsRel',
                     onc='cmds.rowLayout("rowCambiaColor",e=1,bgc=[0.5,0.5,0.5]) ')
    row_ventanaCrearLuz = cmds.columnLayout(p=rowDeColumnas)
    cmds.radioCollection()
    cmds.radioButton('R', label='RELATIVO', ann="RELATIVO", cl='radioAbsRel',
                     onc='cmds.rowLayout("rowCambiaColor",e=1,bgc=[0.18,0.65,0.72]) ')
    row_filtros = cmds.rowLayout(p=rowDeColumnas, nc=21)
    cmds.text(label='               CT ', ann="COLOR TEMPERATURE", p=row_filtros)
    cmds.iconTextButton('b_masCalido', ann='-100', style='iconOnly', image1='arrowDown', c=masCalido, width=15,
                        height=15, p=row_filtros, bgc=(1, 0.2, 0.2))
    crearPopTempColor('b_masCalido')
    tempColor = cmds.floatField('aiColorTemperature', w=40, bgc=(1, 0.2, 0.2), en=0, pre=0, h=15, cc=setTempColor,
                                p=row_filtros)
    ...
    cmds.iconTextButton('b_masFrio', ann='+100', style='iconOnly', image1='arrowUp', c=masFrio, width=15, height=15,
                        p=row_filtros, bgc=(0.2, 0.2, 1))
    crearPopTempColor('b_masFrio')
    # ESTO ESTA DESHABILITADO HASTA QUE ESTE FUNCIONAL
    # filtrosDropMenu = cmds.optionMenuGrp('oM_filtros',annotation="FILTROS",bgc = [0.7,0.7,0.7],w=70,en=0,changeCommand=asignarFiltro,p=row_filtros)
    ...
    frame_2 = cmds.frameLayout('frame_2', label='EXTRAS ARNOLD SETTINGS', borderStyle='in',
                               expandCommand="cmds.frameLayout('frame_2', e=1 , height= cmds.frameLayout('frame_2', q=1 , height= 1)+20 )\ncmds.window('winMLuces', e=1 , resizeToFitChildren=1 )",
                               collapseCommand="cmds.frameLayout('frame_2', e=1 , height= cmds.frameLayout('frame_2', q=1 , height= 1)-20 )\ncmds.window('winMLuces', e=1 , h=cmds.window('winMLuces', q=1 , height= 1)-20 )",
                               collapsable=1, p=win, h=50, w=520)
    row_6 = cmds.rowLayout(p=frame_2, numberOfColumns=21, h=30)
    b_sss = cmds.iconTextButton('b_sss', l='SSS', style='textOnly', image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_SSS.png',
                                c=sss, width=90, height=25, p=row_6, bgc=(0.2, 0.2, 0.2))
    b_disp = cmds.iconTextButton('b_disp', l='DISPLACEMENT', style='textOnly',
                                 image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png', c=disp, width=90, height=25, p=row_6,
                                 bgc=(0.2, 0.2, 0.2))
    b_sub = cmds.iconTextButton('b_sub', l='SUBDIVISION', style='textOnly',
                                image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png', c=sub, width=90, height=25, p=row_6,
                                bgc=(0.4, 0.4, 0.4))
    b_flush = cmds.iconTextButton('b_flush', l='FLUSH CACHE', style='textOnly',
                                  image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png',
                                  c="cmds.arnoldFlushCache( textures=True )", width=90, height=25, p=row_6,
                                  bgc=(0.4, 0.4, 0.4))
    refreshOverride()
    enableDropParent()
    cmds.window('winMLuces', e=1, resizeToFitChildren=1)
