...
def boldOblique():
	...
	if luces!=None:
		for luz in luces:
			...
			cmds.textScrollList ("listaLuces2",e=1,da=1)
			cmds.textScrollList ("listaLuces3",e=1,da=1)
			if cmds.getAttr (luz+".visibility") and cmds.referenceQuery( luz ,isNodeReferenced=1 )==True: #referenciada y activa
			...
			elif cmds.getAttr (luz+".visibility")==False and cmds.referenceQuery( luz ,isNodeReferenced=1 )==True: #referenciada e inactiva
				cmds.textScrollList ("listaLuces1",e=1,si=luz)
				index= cmds.textScrollList ("listaLuces1",q=1,sii=1)[0]
			...
			elif cmds.getAttr (luz+".visibility")==False and cmds.referenceQuery( luz ,isNodeReferenced=1 )==False: #referenciada e inactiva
				cmds.textScrollList ("listaLuces1",e=1,si=luz)
				index= cmds.textScrollList ("listaLuces1",q=1,sii=1)[0]
			...
			elif cmds.getAttr (luz+".visibility")==True and cmds.referenceQuery( luz ,isNodeReferenced=1 )==False: #referenciada e inactiva
				cmds.textScrollList ("listaLuces1",e=1,si=luz)
				index= cmds.textScrollList ("listaLuces1",q=1,sii=1)[0]
...
def construirScrollsConBotones( emparentarA='') :
	...
	r_00 = cmds.rowLayout ('r_00', nc = 5 , p = 'row1' , h = 30 , cw5= [ 50 , 250 , 85 , 100 , 25 ] , ct5= ["both", "both", "both", "both" , "both"])
	...
	b_isolate = cmds.iconTextButton  ('b_isolate',ann='AISLAR LUZ',  style='iconOnly',image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_ISOLATE.png', c = aislarLuz ,width=15,height=25,p=r_00 )
	...
	cmds.textField ( 'buscador' , e=1 , aie= 1 , changeCommand = buscar,  pht="Buscador" )
	boldOblique()

def buscar(*args):#################################################################################################################################
	global buscarPor_Grupo_Luz
	print buscarPor_Grupo_Luz
	recolectorBusqueda=[]
	try:
...
def aislarLuz(*args):
	...
	if isolate == True:
		...
		for light in myLightsOn:
			lightTRF = cmds.listRelatives ( light , p=1 )
			cmds.showHidden( lightTRF , a = True, b = True)
	else:
		myLights = cmds.ls(type='light')
		myLightsOn = cmds.ls(type='light', visible=True)
		mySelection = cmds.ls (selection = True)
		...
		cmds.hide(myLights)
		cmds.showHidden( cmds.textScrollList ('listaLuces1',q=1,si=1) , a = True, b = True)
...
def sss(*args):
	...
	if deactivateSSS == True:
		deactivateSSS = False
		cmds.iconTextButton  ('b_sss' , e=1 , image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_SSS.png')
		cmds.setAttr ( 'defaultArnoldRenderOptions.ignoreSss', 0)
	else:
		deactivateSSS = True
		cmds.iconTextButton  ('b_sss' , e=1 , image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_SSS_OFF.png')
...
def disp(*args):
	...
	if deactivateDisplacement == True:
		deactivateDisplacement = False
		cmds.iconTextButton  ('b_disp' , e=1 , image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png')
		cmds.setAttr ( 'defaultArnoldRenderOptions.ignoreDisplacement', 0)
	else:
		deactivateDisplacement = True
		cmds.iconTextButton  ('b_disp' , e=1 , image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP_OFF.png')
...
def lightListPanel():
	...
	win = cmds.window('winMLuces', title="PH_LUCES! v1.5 BETA - COMPATIBLE CON ARNOLD -", menuBar=0 , w=100 , s=1, height= 100)
	...
	frame_1 = cmds.frameLayout('frame_1', expandCommand= "cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)+80 )" , collapseCommand = "cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)-80 )\ncmds.window('winMLuces', e=1 , h=cmds.window('winMLuces', q=1 , height= 1)-80 )" , label='ATRIBUTOS', borderStyle='in',collapsable=1,p=win , h=100 , w=520 )
	...
	row_filtros = cmds.columnLayout( p = rowDeColumnas )
	...
	cmds.textScrollList ('listaLuces2' , e=1  ,da=1)
	cmds.textScrollList ('listaLuces3' , e=1  ,da=1)
	frame_2 = cmds.frameLayout('frame_2', label='EXTRAS ARNOLD SETTINGS', borderStyle='in', expandCommand= "cmds.frameLayout('frame_2', e=1 , height= cmds.frameLayout('frame_2', q=1 , height= 1)+20 )\ncmds.window('winMLuces', e=1 , resizeToFitChildren=1 )" , collapseCommand = "cmds.frameLayout('frame_2', e=1 , height= cmds.frameLayout('frame_2', q=1 , height= 1)-20 )\ncmds.window('winMLuces', e=1 , h=cmds.window('winMLuces', q=1 , height= 1)-20 )"   ,collapsable=1,p=win , h=50 , w=520 )
	row_6 = cmds.rowLayout ( p=frame_2 , numberOfColumns=21 , h=30 )
	b_sss = cmds.iconTextButton  ('b_sss',ann='SSS',style='iconOnly',image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_SSS.png', c = sss ,width=25,height=25,p=row_6 ,bgc=(0.2,0.2,0.2))
	b_disp = cmds.iconTextButton  ('b_disp',ann='DISPLACEMENT',style='iconOnly',image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png', c = disp ,width=25,height=25,p=row_6 ,bgc=(0.2,0.2,0.2))
	enableDropParent()
	cmds.window('winMLuces', e=1 , resizeToFitChildren=1 )
	cmds.showWindow(win)
