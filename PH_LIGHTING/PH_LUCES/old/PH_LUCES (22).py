...
def setTempColor(*args):
	try:
		...
		for luz in lucesSeleccionadas:
			try:
				if cmds.radioCollection('radioAbsRel',q=1,sl=1)=='A':
					print cmds.floatField ('aiColorTemperature',q=1,v=1)
...
def lightListPanel():
	...
	frame_1 = cmds.frameLayout('frame_1' , h=120, expandCommand= "cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)+80 )" , collapseCommand = "cmds.frameLayout('frame_1', e=1 , height= cmds.frameLayout('frame_1', q=1 , height= 1)-80 )\ncmds.window('winMLuces', e=1 , h=cmds.window('winMLuces', q=1 , height= 1)-80 )" , label='ATRIBUTOS', borderStyle='in',collapsable=1,p=win , w=520 )
	rootDeLaEscena = ""
	roots = cmds.ls('ROOT_*_LGRP',r=1)
	listaDeRootsNoReferenciados=[]
	for root in roots:
	...
	cmds.text(label='Temperatura' , ann="COLOR TEMPERATURE", p=row_filtros)
	cmds.iconTextButton  ('b_masCalido',ann='-100',style='iconOnly',image1='arrowDown', c = masCalido ,width=15,height=15,p=row_filtros,bgc=(1,0.2,0.2))
	...
	cmds.menuItem(l="3K5" , p=popTempColor , c=partial (setTemperaturaColor, 3500) )
	...
	cmds.iconTextButton  ('b_masFrio',ann='+100',style='iconOnly',image1='arrowUp', c = masFrio ,width=15,height=15,p=row_filtros,bgc=(0.2,0.2,1))
	...
	b_flush = cmds.iconTextButton  ('b_flush',l='FLUSH CACHE',style='textOnly',image1='M:\PH_SCRIPTS\ICONS\PH_LUCES_DISP.png', c = "cmds.arnoldFlushCache( textures=True )" ,width=90,height=25,p=row_6 ,bgc=(0.4,0.4,0.4))
	refreshOverride()
	enableDropParent()
	cmds.window('winMLuces', e=1 , resizeToFitChildren=1 )
	cmds.showWindow(win)
