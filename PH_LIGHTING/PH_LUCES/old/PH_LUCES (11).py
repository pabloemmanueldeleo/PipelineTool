...
def refreshui(refrescarPor="",*args):
	...
	camposAtributosV      = {"aiExposure":0 , "aiRadius":0, "aiColorTemperature":None, "color":None, "intensity":0, "aiAngle":0, "aiSamples":0,"emitDiffuse":0,"emitSpecular":0 }
	...
	for luz in lucesSeleccionadas:
		for key in camposAtributos.keys(): # TIENEN EL MISMO VALOR TODOS?
			if (cmds.attributeQuery (key , node=luz+"Shape" , ex=1)):
				if cmds.getAttr ( luz+"Shape."+key ) != camposAtributosV[key]:
...
def maxmin():
	...
	for luz in lucesSeleccionadas:
		for key in camposAtributosMin.keys(): # CAPTURO VALORES
			if (cmds.attributeQuery (key , node=luz+"Shape" , ex=1)):
				camposAtributosMin[key]= [luz, cmds.getAttr ( luz+"Shape."+key ) ]
				camposAtributosMax[key]= [luz, cmds.getAttr ( luz+"Shape."+key ) ]
	for luz in lucesSeleccionadas:
...
def lightListPanel():
	...
	win = cmds.window('winMLuces', title="PH_LUCES! v1.1 BETA - COMPATIBLE CON ARNOLD -", menuBar=0 , w=100 , s=1, height= 100)
