...
def crearGrupo(*args):
	...
	if len( roots ) == 1:
		...
		rootDeEscena = ""
	print "rootDeEscena " , rootDeEscena
	print "listaDeRootsNoReferenciados " , listaDeRootsNoReferenciados
...
def aislarLuz(*args):
	...
	if isolate == True:
		...
		for lights in myLights:
			cmds.showHidden( cmds.ls (type='light') , a = True, b = True)
	else:
		myLights = cmds.ls(type='light')
		myLightsOn = cmds.ls(type='light', visible=True)
		mySelection = cmds.ls (selection = True)
...
def lightListPanel():
	...
	win = cmds.window('winMLuces', title="PH_LUCES! v1.4 BETA - COMPATIBLE CON ARNOLD -", menuBar=0 , w=100 , s=1, height= 100)
